...
In this section, we are going to apply various hierarchical clustering methods to compute dendrogram, such as in figure (TODO:sample dendogram figure) and compare their results.
The first experiment that we are going to run is to differenciate agglomerative and divisive clustering:
- Agglomerative Clustering: also known as a bottom-up approach, starts with singletons and then agglomerate them based on their distance (TODO fn: As we'll see later, various distance computation methods can be used: simple, average, complete or Ward.) until having a group containing the whole set of elements.
- Divisive Clustering: also known as a top-down approach, starts with one group (or cluster) containing every elements then divides them using a splitting method until reaching singletons clusters.

## Agglomerative hierarchical clustering
To experiment the agglomerative hierarchial clustering, we'll use the *agnes* (footnote: https://www.rdocumentation.org/packages/cluster/versions/2.1.2/topics/agnes) package (agglomerative nesting). 
We'll compute agglomerative hierarchial clustering using 4 different distance compuation methods:
- Simple
- Average
- Complete
- Ward

and then compare them using the agglomerative coefficient that \textit{measures the dissimilarity of an object to the first cluster it 
joins, divided by the dissimilarity of the final merger in the cluster analysis, averaged across all samples. 
Low values reflect tight clustering of objects, larger values indicate less well-formed clusters.} \footnote{Source: http://strata.uga.edu/8370/lecturenotes/clusterAnalysis.html}




```{r, include=TRUE}
d <- Prostate_no_outliers_sc
...
After having performed hierarchical clustering using the agglomerative option, it makes sense to apply the divisive option.
To do so we use the diana algorithm.
...
We can compare both methods using a _tanglegram_ which makes us able to compare two dendograms by drawing links between them.
```{r, include=TRUE}
library(dendextend)
...
* Cluster 1 is characterized by positive values of variables 'ap', 'sg', 'sz'; we can also say that variable 'age' is most significantly associated with cluster 1
...
# Partitioning clustering: k-means
Let's now focus on a famous partitioning clustering algorithm: k-means. It's a stable algorithm based on moving average where, at each steps, elements are gathered into the closest group (in terms of Euclidan distance for example). An example of k-means clustering plots can be found in Figure TODO.

Because the algorithm is based on arbitrary starting centers, we will run multiple times in order to make the most generalistic conclusions possible about our data. \\
...
## Finding the number of centers
In order to have the most relevant method, we first need to find the optimal number of clusters (which is the number of centers we will start with). We will use various methods:
...
- Gap statistic method: based on null hypothesis, compares the total within-cluster variation using different k values on various samples of the data, using bootstraping.
...
k-means as we know, it is very sensitive to the initial choiches made regarding the number of clusters we want to find and the 
initial position of the centroids.
Therefore, one possible improvement that can be made, is to integrate a hierarchical clustering approach in the 
initial stage of the k-means algorithm, to better choose the number of clusters we want to look for when applying k-means.
This is what the hierarchical k-means clustering algorithm does.
In the following section, we apply such algorithm, to obtain more robusts clusters with respect to classical k-means.
A
```{r, include=TRUE}
res.hk <-hkmeans(Prostate_no_outliers_sc,2)
```
